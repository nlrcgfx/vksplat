# VkSplat (reference copy)

Frozen reference implementation for regression and comparison. Active rewrite work lives in `next/nlrc_vksplat/` (Phase 2). Baseline tag: `ref-baseline-2026-06-12`.
...
To recompile shaders after update, run `python compile_shaders.py` from the `ref/` directory.

To force recompile all shaders without caching, use `python compile_shaders.py --force` from `ref/`.
