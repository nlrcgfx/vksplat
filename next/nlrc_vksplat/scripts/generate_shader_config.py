...
WORD_BYTES = 4
TENSOR_BWD_ALPHA_WORDS = 1
TENSOR_BWD_SCREEN_POSITION_WORDS = 2
TENSOR_BWD_COVARIANCE_WORDS = 4
TENSOR_BWD_COLOR_WORDS = 3
TENSOR_BWD_PER_SPLAT_SCREEN_WORDS = 2
TENSOR_BWD_NO_SUBGROUP_GRADIENT_WORDS = 6
TENSOR_BWD_NO_SUBGROUP_COLOR_WORDS = 3
TENSOR_BWD_GROUP_REDUCE_GRADIENT_WORDS = 6
TENSOR_BWD_SUBGROUP_COLOR_REDUCE_WORDS = 3
...
def _tensor_configs(values: dict[str, int]) -> list[dict[str, int]]:
    ...
    for idx in range(count):
        ...
        words = 0
        words += tile_size * TENSOR_BWD_ALPHA_WORDS
        words += tile_size * TENSOR_BWD_SCREEN_POSITION_WORDS
        words += tile_size * TENSOR_BWD_COVARIANCE_WORDS
        words += tile_size * TENSOR_BWD_COLOR_WORDS
        words += tile_size * splat_batch * TENSOR_BWD_PER_SPLAT_SCREEN_WORDS
        if not use_subgroup:
            words += tile_size * TENSOR_BWD_NO_SUBGROUP_GRADIENT_WORDS
            words += tile_size * TENSOR_BWD_NO_SUBGROUP_COLOR_WORDS
        if group_reduce > 0:
            words += tile_size * TENSOR_BWD_GROUP_REDUCE_GRADIENT_WORDS
            if use_subgroup:
                words += tile_size * TENSOR_BWD_SUBGROUP_COLOR_REDUCE_WORDS
        ...
        configs.append(
            {
                ...
                "min_shared_mem": words * WORD_BYTES,
            }
        )
    return configs
