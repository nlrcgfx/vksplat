...
TEST_CASE("Golden float compare passes within epsilon", "[host]") {
  ...
  auto actual_span = make_span(actual);

  const std::vector<float> expected{1.0F, 2.0F, 3.0F};
  auto expected_span = make_span(expected);

  REQUIRE(tests::compare_float_buffers(expected_span, actual_span));
}
...
TEST_CASE("Golden float compare fails outside epsilon", "[host]") {
  ...
  REQUIRE_FALSE(tests::compare_float_buffers(expected_span, actual_span, epsilon, &message));
...
TEST_CASE("NaN detection rejects non-finite values", "[host]") {
  ...
  auto values_span = make_span(values);

  REQUIRE_THROWS_AS(tests::assert_no_nan_inf(values_span), std::runtime_error);
}
