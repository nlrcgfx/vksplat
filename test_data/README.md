...
4. Re-check generated binary sizes, especially `D_cumsum_multi_block_two_level`.
...
| `D_cumsum_single_pass_near_block` | `cumsum_single_pass` GPU utility shader | `B - 1` values, currently 511, generated by `(index % 7) - 3` | Inclusive prefix sum | Single-pass behavior just below `VKSPLAT_CUMSUM_BLOCK_SIZE`. |
| `D_cumsum_single_pass_exact_block` | `cumsum_single_pass` GPU utility shader | `B` ones, currently 512 | `[1..B]` | Single-pass behavior at `VKSPLAT_CUMSUM_BLOCK_SIZE`. |
| `D_cumsum_multi_block` | `cumsum_block_scan` + `cumsum_scan_block_sums` + `cumsum_add_block_offsets` | `B + 1` signed cyclic values, currently 513, generated by `(index % 11) - 5` | Inclusive prefix sum | One-level multi-block cumsum path with signed values. |
| `D_cumsum_multi_block_two_level` | two-level multi-phase cumsum with `block_sums2` | `B * B + 1` signed cyclic values, currently 262145, generated by `(index % 5) - 2` | Inclusive prefix sum | Two-level `_cumsum_blockSums2` path above `VKSPLAT_CUMSUM_BLOCK_SIZE * VKSPLAT_CUMSUM_BLOCK_SIZE`. |
| `D_sum` | `sum` GPU utility shader | `[1, 0, 2, 3]` | `[6]` | Small integer reduction. |
| `D_sum_multi_block` | `sum` GPU utility shader | `S + 1` ones, currently 513 | `[S + 1]` | Multi-workgroup reduction path using atomic accumulation. |
...
Generated minimal `.bin` files are committed. `D_cumsum_multi_block_two_level`
