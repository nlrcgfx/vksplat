...
- `shader_binding_contracts.json`: schema-v2 binding name snapshot, fixture composite contracts, and fixture/golden route table
...
- `binding_contract`: JSON route result that selected the manifest `bindings` list, such as `shaders/sum` or `fixture_contracts/cumsum_output`.
- `bindings`: descriptor binding order used by the shader and the test. Generated from `shader_binding_contracts.json`, not hand-edited per manifest.
...
2. Edit `test_data/shader_binding_contracts.json` for new fixture/golden routes or fixture-only composite contracts.
3. Edit `test_data/generate_fixtures.py`.
4. Regenerate files:
...
5. Check config mirrors and generated-data drift:
...
in `--check` mode with the configured Python interpreter. It also registers
`nlrc_vksplat_shader_binding_contracts_check`, which verifies the JSON `shaders`
snapshot against the C++ descriptor registry and validates route references.
The fixture-generation check first verifies
that Python mirror constants in `generate_fixtures.py` match evaluated
`VKSPLAT_*` values from `nlrc_vksplat_config.hpp`, then compares generated bytes
against the committed fixtures and goldens. If `python` is not on `PATH`, use the
interpreter from `build/<preset>/CMakeCache.txt` or rerun CMake with
...
`generate_fixtures.py --check` enforces the mirrored constants listed there before
checking fixture byte drift.
...
`--write` fails before generating files if the Python mirror constants do not
match `nlrc_vksplat_config.hpp`.

3. Check config mirrors and generated-data drift:
...
When future Phase 4/5 fixtures hardcode SSIM or tensor-backward config values,
add those Python constants to the mirror guard in `generate_fixtures.py` in the
same change.
...
- Add or update JSON routes and fixture-only contracts in `shader_binding_contracts.json`; do not duplicate route logic in Python or C++.
- Keep buffers minimal and deterministic.
- Regenerate with `--write`, then verify with `--check`.
- Keep descriptor-order `bindings` generated from the selected JSON `binding_contract`.
